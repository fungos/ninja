...
bool DependencyScan::RecomputeOutputDirty(Edge* edge,
 ...
 /*
  * FIXME: Currently we comment this step out as we use DBS Script to build our code.
  * this means that ninja never has the opportunity to issue the compile command and
  * calculate its hash to its log. We may add a parameter to do a kind of dry-run with
  * log or anything else specific to DBS. I will check this in the future.
  */
 /*
...
bool ImplicitDepLoader::LoadDepFile(Edge* edge, const string& path,
  ...
  /*
   * DANNY: This fix the one problem with .d generated by ARMCC and we do not need to fix each dependency path slashes.
   */
  string opath = IncludesNormalize::Normalize(StringPiece(first_output->path()).AsString(), NULL);
  string depout = IncludesNormalize::Normalize(depfile.out_.AsString(), NULL);
  if (opath != depout) {
